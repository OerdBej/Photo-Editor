...
const saveImage = () => {
    //  canvas.getContext return a drawing context for the canvas
    const canvas = document.createElement('canvas');
    // return the draw on the canvas
    const getContext = canvas.getContext('2d');
    // canva to match height and weight of the image
    canvas.width = previewImg.naturalWidth;
    canvas.height = previewImg.naturalHeight;
    // applying users selected filters to canvas filter
    getContext.filter = `brightness(${brightness}%) saturate(${saturation}%) invert(${inversion}%) grayscale(${grayscale}%)`;
    // This method will make the canvas to the center
    getContext.translate(canvas.width / 2, canvas.height / 2);
    // if the rotating value is not 0, then rotate the canvas
    if (rotate !== 0) {
        getContext.rotate((rotate * Math.PI) / 180);
    }
    getContext.scale(flipHorizontal, flipVertical);
